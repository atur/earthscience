...
\restrict KkSAu06IfpAXezE755uhNr4IhVJq93Rpf7tteMbEiFGRhe3Zz2hvlj7xlHUkXEP
...
CREATE FUNCTION dbnext.prevent_last_determination_delete() RETURNS trigger
    ...
    AS $$
BEGIN
    PERFORM 1 FROM dbnext.project_record
    WHERE id = OLD.id_project_record
    FOR UPDATE;
    IF NOT FOUND THEN
        RETURN NULL;
    END IF;
...
COMMENT ON FUNCTION dbnext.prevent_last_determination_delete() IS 'Deferred constraint trigger function: once a project_record has determinations, the last one cannot be removed (by DELETE or by re-pointing via UPDATE) while the record exists. Locks the parent project_record (FOR UPDATE) against concurrent removal races.';
...
CREATE FUNCTION dbnext.prevent_last_project_record_project_delete() RETURNS trigger
    ...
    AS $$
BEGIN
    -- Lock the parent record: concurrent transactions removing associations
    -- of the same record serialize here, so the second one sees the first
    -- one's committed deletion when it re-checks below.
    PERFORM 1 FROM dbnext.project_record
    WHERE id = OLD.id_project_record
    FOR UPDATE;
    IF NOT FOUND THEN
        -- parent record deleted in this transaction (cascade delete)
        RETURN NULL;
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM dbnext.project_record_project
...
COMMENT ON FUNCTION dbnext.prevent_last_project_record_project_delete() IS 'Deferred constraint trigger function: blocks removing (by DELETE or by re-pointing via UPDATE) the last project association of a still-existing project_record. Locks the parent project_record (FOR UPDATE) so concurrent transactions removing different associations of the same record serialize instead of both passing the EXISTS check.';


--
-- Name: set_date_bounds(); Type: FUNCTION; Schema: dbnext; Owner: postgres
--

CREATE FUNCTION dbnext.set_date_bounds() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF NEW.date_start IS NULL AND NEW.date_end IS NOT NULL THEN
        NEW.date_start := NEW.date_end;
    ELSIF NEW.date_end IS NULL AND NEW.date_start IS NOT NULL THEN
        NEW.date_end := NEW.date_start;
    END IF;
    RETURN NEW;
END;
$$;
...
ALTER FUNCTION dbnext.set_date_bounds() OWNER TO postgres;

--
-- Name: FUNCTION set_date_bounds(); Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON FUNCTION dbnext.set_date_bounds() IS 'Mirrors the missing bound onto the other one so date_start/date_end are either both set or both NULL. Prevents half-open date_range values like [start,) that match every later instant.';
...
CREATE TABLE dbnext.data_definition (
    ...
    CONSTRAINT ck_data_definition_type CHECK ((type = ANY (ARRAY['int'::text, 'string'::text, 'date'::text])))
);
...
--
-- Name: COLUMN data_definition.predefined_values; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.data_definition.predefined_values IS 'Allowed values for this custom field (e.g. the choices of a dropdown), in display order. NULL = value is not restricted to a list.';
...
ALTER TABLE dbnext.data_definition ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.data_definition_id_seq
    START WITH 28
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.data_group (
    ...
    name text NOT NULL,
    description text NOT NULL,
    CONSTRAINT ck_data_group_no_self_parent CHECK ((id_parent <> id))
);
...
ALTER TABLE dbnext.data_group ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.data_group_id_seq
    START WITH 29
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.external_identifier ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.external_identifier_id_seq
    START WITH 82
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.external_identifier_item ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.external_identifier_item_id_seq
    START WITH 5
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.external_identifier_project_record ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.external_identifier_project_record_id_seq
    START WITH 82
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.item (
    ...
    CONSTRAINT ck_item_language CHECK ((language ~ '^[a-z]{2}$'::text))
);
...
--
-- Name: COLUMN item.data; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.item.data IS 'Global, list-independent custom-field values of the item (allowed fields defined by item.id_data_group). List-specific additional values live in item_list_item.data.';


--
-- Name: COLUMN item.id_data_group; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.item.id_data_group IS 'Data group defining the allowed custom fields in item.data. Different kinds of items (e.g. biodiversity taxa, specimen stores) reference different data groups. NULL = the item has no global custom fields. The application is responsible for keeping items of the same kind on the same data group.';
...
ALTER TABLE dbnext.item ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_id_seq
    START WITH 1559
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.item_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_keyword_id_seq
    START WITH 7
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.item_list (
    ...
    id_parent bigint,
    data jsonb NOT NULL,
    item_list_id_data_group bigint,
    item_list_item_id_data_group bigint,
    created_by bigint,
    modified_by bigint,
    date_create timestamp with time zone DEFAULT now() NOT NULL,
    date_modify timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_item_list_no_self_parent CHECK ((id_parent <> id))
);
...
COMMENT ON COLUMN dbnext.item_list.item_list_id_data_group IS 'reference to the data_group in which custom fields are defined in the data field of item_list. NULL = the list has no custom fields.';


--
-- Name: COLUMN item_list.item_list_item_id_data_group; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.item_list.item_list_item_id_data_group IS 'reference to the data group in which the additional list-specific custom fields are defined for items of this list. The values are stored in item_list_item.data (not in item.data, which holds the list-independent global custom fields). NULL = items of this list have no list-specific custom fields.';
...
ALTER TABLE dbnext.item_list ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_list_id_seq
    START WITH 38
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.item_list_item (
    ...
    CONSTRAINT ck_item_list_item_no_self_parent CHECK ((id_parent <> id))
);
...
--
-- Name: COLUMN item_list_item.data; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.item_list_item.data IS 'List-specific custom-field values for this item in this list; allowed fields are defined by the data_group referenced in item_list.item_list_item_id_data_group. Additive only: keys must not overlap with the item''s global custom fields in item.data (disjointness is enforced by the application, not the DB). Effective values for an item in a list = item.data || item_list_item.data.';
...
ALTER TABLE dbnext.item_list_item ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_list_item_id_seq
    START WITH 1559
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.item_list_item_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_list_item_keyword_id_seq
    START WITH 16
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.item_list_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.item_list_keyword_id_seq
    START WITH 25
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.keyword (
    ...
    date_create timestamp with time zone DEFAULT now() NOT NULL,
    date_modify timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_keyword_no_self_parent CHECK ((id_parent <> id))
);
...
ALTER TABLE dbnext.keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.keyword_id_seq
    START WITH 269
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.media ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.media_id_seq
    START WITH 14
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.media_item ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.media_item_id_seq
    START WITH 7
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.project (
    ...
    date_modify timestamp with time zone DEFAULT now() NOT NULL,
    id_user bigint,
    CONSTRAINT ck_project_no_self_parent CHECK ((id_parent <> id))
);
...
ALTER TABLE dbnext.project ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_id_seq
    START WITH 18
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_item_list ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_item_list_id_seq
    START WITH 16
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_keyword_id_seq
    START WITH 13
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COMMENT ON COLUMN dbnext.project_record.date_range IS 'Generated range column from date_start and date_end. Enables range operators (@>, &&, etc.) for temporal queries. Always closed [start,end] or NULL — never half-open, because trg_set_date_bounds keeps both bounds either set or NULL. NULL means the record has no temporal reference and matches no range query.';


--
-- Name: COLUMN project_record.date_start; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.project_record.date_start IS 'Start of the recording event. If only one bound is supplied, trg_set_date_bounds mirrors it onto the other, so date_start and date_end are either both set or both NULL.';


--
-- Name: COLUMN project_record.date_end; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.project_record.date_end IS 'End of the recording event. If only one bound is supplied, trg_set_date_bounds mirrors it onto the other, so date_start and date_end are either both set or both NULL.';
...
COMMENT ON TABLE dbnext.project_record_data_group IS 'Assigns the data_groups whose custom fields apply to the records of a project (M:N, ordered by sort). Note: links project - not project_record - to data_group; records inherit the union of the groups of their projects via project_record_project and are never linked to a data_group directly. The values live in project_record.data.';


--
-- Name: COLUMN project_record_data_group.sort; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON COLUMN dbnext.project_record_data_group.sort IS 'Ordering of the data groups within a project (application/UI order of the record custom-field groups).';
...
ALTER TABLE dbnext.project_record_data_group ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_data_group_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COMMENT ON COLUMN dbnext.project_record_determination.id_determination_method IS 'FK to keyword table. Defines the method used (e.g. morphological, molecular). Managed as keywords. Not restricted by the DB — the application must limit choices to keywords from the appropriate subtree.';
...
ALTER TABLE dbnext.project_record_determination ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_determination_id_seq
    START WITH 12178
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_determination_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_determination_keyword_id_seq
    START WITH 11
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_geometry ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_geometry_id_seq
    START WITH 12128
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_id_seq
    START WITH 12186
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
COMMENT ON COLUMN dbnext.project_record_identifier.id_keyword IS 'Defines the identifier type (e.g. Catalog Number, Accession Number). References keyword table. Not restricted by the DB — the application must limit choices to keywords from the appropriate subtree.';
...
ALTER TABLE dbnext.project_record_identifier ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_identifier_id_seq
    START WITH 12040
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_keyword_id_seq
    START WITH 24097
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_media ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_media_id_seq
    START WITH 14
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.project_record_parent (
    ...
    id_keyword bigint,
    description text,
    CONSTRAINT ck_project_record_parent_no_self CHECK ((id_project_record <> id_project_record_parent))
);
...
COMMENT ON COLUMN dbnext.project_record_parent.id_keyword IS 'Optional keyword defining the type of hierarchical relationship. Not restricted by the DB — the application must limit choices to keywords from the appropriate subtree.';
...
ALTER TABLE dbnext.project_record_parent ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_parent_id_seq
    START WITH 12070
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_project ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_project_id_seq
    START WITH 12183
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE dbnext.project_record_record (
    ...
    id_keyword bigint NOT NULL,
    description text,
    CONSTRAINT ck_project_record_record_no_self CHECK ((id_project_record_1 <> id_project_record_2))
);
...
COMMENT ON COLUMN dbnext.project_record_record.id_keyword IS 'Keyword defining the type of relationship (e.g. duplicate of, host of, same event). Not restricted by the DB — the application must limit choices to keywords from the appropriate subtree.';
...
ALTER TABLE dbnext.project_record_record ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_record_id_seq
    START WITH 9064
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_user ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_user_id_seq
    START WITH 6018
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.project_record_user_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.project_record_user_keyword_id_seq
    START WITH 2018
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.users ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.users_id_seq
    START WITH 22
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE dbnext.users_keyword ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME dbnext.users_keyword_id_seq
    START WITH 6
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE INDEX idx_date_start_date_end ON dbnext.project_record USING btree (date_start, date_end);
...
--
-- Name: idx_ili_data; Type: INDEX; Schema: dbnext; Owner: postgres
--

CREATE INDEX idx_ili_data ON dbnext.item_list_item USING gin (data jsonb_path_ops) WITH (fastupdate='true');
...
--
-- Name: idx_item_data_group; Type: INDEX; Schema: dbnext; Owner: postgres
--

CREATE INDEX idx_item_data_group ON dbnext.item USING btree (id_data_group);
...
CREATE INDEX idx_item_list_item_data_group ON dbnext.item_list USING btree (item_list_item_id_data_group);
...
--
-- Name: idx_prg_project_record; Type: INDEX; Schema: dbnext; Owner: postgres
--

CREATE INDEX idx_prg_project_record ON dbnext.project_record_geometry USING btree (id_project_record);
...
--
-- Name: idx_users_created_by; Type: INDEX; Schema: dbnext; Owner: postgres
--

CREATE INDEX idx_users_created_by ON dbnext.users USING btree (created_by);


--
-- Name: idx_users_modified_by; Type: INDEX; Schema: dbnext; Owner: postgres
--

CREATE INDEX idx_users_modified_by ON dbnext.users USING btree (modified_by);
...
--
-- Name: INDEX uq_users_email_lower; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON INDEX dbnext.uq_users_email_lower IS 'Case-insensitive uniqueness of users.email. Replaces the redundant uq_users_email constraint: two e-mails differing only in case already fail on this index.';
...
CREATE CONSTRAINT TRIGGER trg_prevent_last_determination_delete AFTER DELETE OR UPDATE OF id_project_record ON dbnext.project_record_determination DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION dbnext.prevent_last_determination_delete();
...
CREATE CONSTRAINT TRIGGER trg_prevent_last_project_association_delete AFTER DELETE OR UPDATE OF id_project_record ON dbnext.project_record_project DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION dbnext.prevent_last_project_record_project_delete();
...
--
-- Name: project_record trg_set_date_bounds; Type: TRIGGER; Schema: dbnext; Owner: postgres
--

CREATE TRIGGER trg_set_date_bounds BEFORE INSERT OR UPDATE ON dbnext.project_record FOR EACH ROW EXECUTE FUNCTION dbnext.set_date_bounds();
...
--
-- Name: users trg_set_date_modify; Type: TRIGGER; Schema: dbnext; Owner: postgres
--

CREATE TRIGGER trg_set_date_modify BEFORE UPDATE ON dbnext.users FOR EACH ROW EXECUTE FUNCTION dbnext.set_date_modify();
...
ALTER TABLE ONLY dbnext.project_record_geometry
    ADD CONSTRAINT fk_geometry_record FOREIGN KEY (id_project_record) REFERENCES dbnext.project_record(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY dbnext.item_list_item
    ADD CONSTRAINT fk_id_accepted_id FOREIGN KEY (id_accepted, id_item_list) REFERENCES dbnext.item_list_item(id, id_item_list) DEFERRABLE;


--
-- Name: CONSTRAINT fk_id_accepted_id ON item_list_item; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON CONSTRAINT fk_id_accepted_id ON dbnext.item_list_item IS 'Composite FK: accepted entry must belong to the same list. DEFERRABLE. Validated (data was clean when converted from the former NOT VALID single-column FK).';
...
ALTER TABLE ONLY dbnext.item_list_item
    ADD CONSTRAINT fk_ili_identity FOREIGN KEY (id_identity, id_item_list) REFERENCES dbnext.item_list_item(id, id_item_list) DEFERRABLE;


--
-- Name: CONSTRAINT fk_ili_identity ON item_list_item; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON CONSTRAINT fk_ili_identity ON dbnext.item_list_item IS 'Composite FK: identity target must belong to the same list. DEFERRABLE.';
...
ALTER TABLE ONLY dbnext.item_list_item
    ADD CONSTRAINT fk_ili_parent FOREIGN KEY (id_parent, id_item_list) REFERENCES dbnext.item_list_item(id, id_item_list) DEFERRABLE;


--
-- Name: CONSTRAINT fk_ili_parent ON item_list_item; Type: COMMENT; Schema: dbnext; Owner: postgres
--

COMMENT ON CONSTRAINT fk_ili_parent ON dbnext.item_list_item IS 'Composite FK: parent must belong to the same list. DEFERRABLE so multi-step corrections (moving an entry to another list) can be done in one transaction.';
...
--
-- Name: item fk_item_data_group; Type: FK CONSTRAINT; Schema: dbnext; Owner: postgres
--

ALTER TABLE ONLY dbnext.item
    ADD CONSTRAINT fk_item_data_group FOREIGN KEY (id_data_group) REFERENCES dbnext.data_group(id);
...
--
-- Name: item_list fk_item_list_item_group; Type: FK CONSTRAINT; Schema: dbnext; Owner: postgres
--

ALTER TABLE ONLY dbnext.item_list
    ADD CONSTRAINT fk_item_list_item_group FOREIGN KEY (item_list_item_id_data_group) REFERENCES dbnext.data_group(id);
...
--
-- Name: users fk_users_created_by; Type: FK CONSTRAINT; Schema: dbnext; Owner: postgres
--

ALTER TABLE ONLY dbnext.users
    ADD CONSTRAINT fk_users_created_by FOREIGN KEY (created_by) REFERENCES dbnext.users(id);


--
-- Name: users fk_users_modified_by; Type: FK CONSTRAINT; Schema: dbnext; Owner: postgres
--

ALTER TABLE ONLY dbnext.users
    ADD CONSTRAINT fk_users_modified_by FOREIGN KEY (modified_by) REFERENCES dbnext.users(id);
...
\unrestrict KkSAu06IfpAXezE755uhNr4IhVJq93Rpf7tteMbEiFGRhe3Zz2hvlj7xlHUkXEP
